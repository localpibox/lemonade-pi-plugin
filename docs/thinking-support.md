...
**File:** `lib/models.ts`  
**Commit:** uncommitted (lpb-dev branch)  
**Line:** ~155
...
**Method:** Send raw HTTP requests to `http://192.168.0.13:13305/v1/chat/completions` with the exact parameters the harness sends post-fix.
...
`test/model-mapping.test.ts` — 10/10 pass:
...
## 5. Server Version Analysis (b10375)

Your lemonade/llama.cpp build is **b10375**. Here's what's included and what's still needed:
...
|---|---|---|---|
| **Per-request reasoning toggle** | PR #22336 not yet merged | Update lemonade when PR lands | **P0** |
| **Configure `--reasoning-budget-message`** | Available but not configured | Add to server startup config | **P1** |
...
### P0: Per-request reasoning toggle (highest impact)

**The problem:** Hybrid models like Qwen3.6 (thinking+non-thinking modes) currently require two separate server processes — one started with `--reasoning on`, one with `--reasoning off`. This doubles RAM usage for a 35B model.
...
### P1: Configure `--reasoning-budget-message` (easy, 10% quality boost)
